...
var continents:object = []; // [{"Asia":["Afghanistan","Armenia",...]}, ...]
...
window.addEventListener("load", (event) => {
    ...
    // A COMPLETER <---
    // --> A COMPLETER
    let tbody = document.getElementById("main")?.firstChild?.firstChild as HTMLElement;
    tbody.childNodes.forEach(tr => {
        tr.childNodes.forEach(td => {
            countries["names"].push(td.innerText);
        });
    });
